...
Agenda.prototype.jobs = function() {
  ...
  return this._db.find.apply(this._db, args);
...
Agenda.prototype.every = function(interval, names, data, startTime, endTime) {
  ...
  function createJob(interval, name, data) {
    ...
    job.attrs.status = 1;  //normal status
...
Agenda.prototype._findAndLockNextJob = function(jobName, definition, cb) {
  ...
  //console.log('_findAndLockNextJob ', jobName, this._nextScanAt);
  this._db.findOneAndUpdate(
    {
      $or: [
        ...
        {name: jobName, 
            startTime: {$lte: now}, endTime: {$gt: now},
            lockedAt: {$lte: lockDeadline}, nextRunAt: {$lte: this._nextScanAt}, disabled: { $ne: true }}
      ]
    },
    {$set: {lockedAt: now}},
    {'new': true, 'sort': {'priority': -1}},
