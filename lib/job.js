...
Job.prototype.computeNextRunAt = function() {
  ...
  if (interval) {
    ...
    if(!this.attrs.startTime || this.attrs.startTime <= new Date()){
      computeFromInterval.call(this);
    } else{
      computeFromStartTime.call(this);
    }
  } else if (repeatAt) {
  ...
  function computeFromStartTime() {
    this.attrs.lastRunAt = null;
    this.attrs.nextRunAt = this.attrs.startTime;
  }
};

Job.prototype.repeatEvery = function(interval) {
  this.attrs.startTime = new Date(1990, 1, 1);
  this.attrs.endTime = new Date(2099, 1, 1);
  this.attrs.status = 1;
